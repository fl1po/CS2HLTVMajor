...
function getTeamNodes(teams) {
  ...
  if (isOverview) {
    return teams.map((team) =>
      Array.from(team.childNodes).filter(
...
function getGroupStage() {
  const title =
    document.getElementsByClassName("event-hub-title")[0]?.innerText;
  if (!title) return {};
  const playoffNode = document.getElementsByClassName(
    "slotted-bracket-placeholder",
  )?.[0];
  const playoffData = JSON.parse(localStorage.getItem(title))?.[title]
    .champions;
  ...
  if (playoffNode) {
    ...
    if (rounds.length) {
      ...
      const roundMatches = [...slot.getElementsByClassName("slot-wrapper")].map(
        (node) => node.getElementsByClassName("match")[0],
      );
  ...
  const hasPlayoffData =
    Boolean(
      (playoffData && Object.values(playoffData).length) || playoffNode,
    ) && !hasUndecidedData;
  if (hasPlayoffData)
    return {
...
function setData() {
  ...
  const allTeams =
    [
      ...document.getElementsByClassName(
        isOverview ? "team text-ellipsis" : "match-teamname",
      ),
    ] || [];
  ...
  function setGroupData() {
    ...
    const results = [
      ...teams.map(
        (team) => team.parentElement.parentElement.parentElement.children,
      ),
    ].map(
      (nodes) =>
        [...nodes].find((node) => node.className === "points cell-width-record")
          .innerText,
    );
  ...
  function setPlayoffData() {
    ...
    rounds.forEach((round, roundId) => {
      ...
      const roundName =
        round.getElementsByClassName("round-header")[0].innerText;
      roundMatches.forEach((match, matchIndex) => {
        ...
        teamNodes.forEach((teamNode, teamId) => {
          ...
          if (
            ...
            const oppositeTeam =
              teamNodes.find((node) => node.id !== teamNode.id) || {};
  ...
  if (isPlayoff) {
    if (!isOverview) {
      const matchesNodes = [
        ...document.getElementsByClassName("liveMatch"),
        ...document.getElementsByClassName("upcomingMatch"),
      ];
      matchesNodes.forEach((matchNode) => {
        const teamNodes = [
          ...matchNode.getElementsByClassName("matchTeamName"),
        ];
        const teamNames = teamNodes.map((node) => node.innerText);
  ...
  teamSets
    ...
    .forEach(({ teamsCond, style }) => {
      const allTabsTeams = getTeamNodes(allTeams)
        .map((team) => (isOverview ? team[0].firstChild : team))
        .filter(teamsCond);
